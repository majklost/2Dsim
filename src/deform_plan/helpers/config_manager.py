...
SUBSAMPLER = {'SUBSAMPLER':
    { # Configuration for sampling guiding paths
        'ITERATIONS': 10000, #iterations of subsampler
...
class ConfigManager:
    ...
    def __deepcopy__(self, memodict={}):
        raise NotImplementedError("Deepcopy is not supported for ConfigManager")
    ...
    def check_unused(self):
        ...
        if unused_keys:
            unused = "\n".join(f"  - {key}: {self._current_config[key]}" for key in unused_keys)
            warnings.warn(f"The following configuration values were not used: \n{unused}")
