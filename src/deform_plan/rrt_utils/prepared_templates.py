...
def get_planning_fncs_cost(cfg: ConfigManager, movable_idx: int, cost_fnc, max_cost):
    ...
    control_fnc["guider"] = fct.make_guider_cutoff(
...
def prepare_goal_bias_sampler(cfg: ConfigManager, goal_points, subsampler):
    ...
    sampler = GoalBiasSampler(subsampler, goal_points,
                              cfg.GOAL_BIAS, verbose=True)
    return sampler
...
def prepare_trrt_wrapper(cfg, storage_utils, cost_fnc, goal):
    ...
    storage_wrapper = TRRT(storage_utils, trrt_utils, goal,
                           cfg.REACHED_THRESHOLD, controlled_idxs=cfg.CONTROL_IDXS, verbose=True)
    return storage_wrapper
...
def prepare_guiding_paths(cfg,
    ...
    for i in range(cfg.SUBSAMPLER_RUNS):
        rh = RectHeuristic(fixed_objects, sampler_data,
                           cfg.CFG, show_sim_bool, show_sim_bool)
        path = rh.run()
        if path:
            paths.append(path)
    # for p in paths[0]:
    ...
    heuristic_sampler = HeuristicSampler(
        child_sampler, child_wrapper, paths, cfg.PATH_PROB, cfg.STD_DEV)
