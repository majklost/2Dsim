...
class EmptyWorld:
    ...
    def __init__(self, cfg: ConfigManager):
        ...
        self._sampler = BezierSampler(cfg.CABLE_LENGTH, cfg.SEGMENT_NUM, np.array(
            [0, 0, 0]), np.array([cfg.CFG["width"], cfg.CFG["height"], 2 * np.pi]))
    ...
    def _add_basic(self):
        ...
        end_platform = Rectangle(
            np.array([WIDTH-EMPTY//2-10, HEIGHT//2]), EMPTY, HEIGHT, KINEMATIC, True)
    ...
    def _add_cable(self):
        self.cable = Cable(START, self.cfg.CABLE_LENGTH,
                           self.cfg.SEGMENT_NUM, thickness=5, angle=np.pi)
    ...
    def get_sim(self):
        sim = Simulator(self.cfg.CFG, self.movable, self.fixed,
                        threaded=self.cfg.THREADED_SIM, unstable_sim=self.cfg.UNSTABLE_SIM)
