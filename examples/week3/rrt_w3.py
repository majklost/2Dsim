...
from examples.week3.w3_utils import make_creased_cost, make_cost_fnc
...
if __name__ == "__main__":
    ...
    obstacle_g = RandomObstacleGroup(np.array([50, 300]), 200, 200, 4, 2, radius=100)
    ...
    sub_sampler = prepare_standard_sampler(cfg)
    goal_points = sub_sampler.sample(x=280,y=730,angle=0)
    ...
    if cfg.USE_GOAL_BIAS:
        sampler = prepare_goal_bias_sampler(cfg,goal_points)
    else:
    ...
    rp = ReplayablePath(
        sim, path, goal, control_fnc["guider"], control_fnc["reached_condition"], cfg.GUIDER_PERIOD,{
            "one_time_draw": draw,
