...
# from deform_plan.rrt_utils.dist import distance_inner_cython as distance_inner
...
def prepare_goal_bias_sampler(cfg:ConfigManager,goal_points):
...
def prepare_guiding_paths(cfg,
    ...
    :param reached_fnc: fnc (simNode) -> Bool
    ...
    for i in range(cfg.SUBSAMPLER_RUNS):
        path = RectHeuristic(fixed_objects,sampler_data,cfg.CFG,show_sim_bool,show_sim_bool)
        paths.append(path)
    print("Found ", len(paths), " paths")
    heuristic_sampler = HeuristicSampler(child_sampler,child_wrapper,paths,reached_fnc,cfg.PATH_PROB,cfg.STD_DEV)
